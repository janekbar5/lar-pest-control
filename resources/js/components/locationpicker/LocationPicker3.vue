...
<div>
      ...
      <br>
	  addressObject: {{ lat }}  {{ lng }}
      <div id="locationPicker" style="width: auto; height: 500px;"></div>	
 </div>     
</template>
	 ...
	 props: {    
      ...
      methods: {   
        ...
        var greenIcon = L.icon({
         ...
         if(!this.marker){
			this.marker = new L.marker([this.lat, this.lng], {icon: greenIcon,draggable:'true'});
			this.circle = new L.circle([this.lat, this.lng], this.r, {
				weight: 2
			});
		}else{
		...
		this.marker.on('dragend', function(e){
			_this.circle.setLatLng(e.target.getLatLng());
            _this.myMap.setView(e.target.getLatLng());           
            _this.passToParent(e.target.getLatLng())   
            //console.log(e.target)        
        });
            
		this.myMap.addLayer(this.marker);
        this.myMap.addLayer(this.circle);   
        }
      }
    };
    </script>
